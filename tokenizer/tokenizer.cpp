                    ...
                    if (!current_char.has_value()){
                        ...
                        if(len>10||(len==10&&token_string.at(index)>'2'))
                            ...
                            if(token_integer<0)
                                ...
                                return std::make_pair(std::make_optional<Token>(TokenType::UNSIGNED_INTEGER,token_string,pos,currentPos()), std::optional<CompilationError>());
                        }

//                        for(unsigned int i=0;i<token_string.length();i++){
//                            if(!isdigit(token_string.at(i))){
//                                return std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(pos,ErrorCode::ErrInvalidInput));
//                            }
//                        }
                        return std::make_pair(std::make_optional<Token>(TokenType::UNSIGNED_INTEGER,token_string,pos,currentPos()), std::optional<CompilationError>());
...
//                        return std::make_pair(std::make_optional<Token>(TokenType::UNSIGNED_INTEGER,token_string,pos,currentPos()), std::optional<CompilationError>());
                ...
                case IDENTIFIER_STATE: {
                    ...
                    if (!current_char.has_value()){
                        ...
                        if(token_string.compare("begin")==0){
                            return std::make_pair(std::make_optional<Token>(TokenType::BEGIN,token_string,pos,currentPos()), std::optional<CompilationError>());
                        }
                        else if(token_string.compare("end")==0){
                            return std::make_pair(std::make_optional<Token>(TokenType::END,token_string,pos,currentPos()), std::optional<CompilationError>());
                        }
                        else if(token_string.compare("const")==0){
                            return std::make_pair(std::make_optional<Token>(TokenType::CONST,token_string,pos,currentPos()), std::optional<CompilationError>());
                        }
                        else if(token_string.compare("var")==0){
                            return std::make_pair(std::make_optional<Token>(TokenType::VAR,token_string,pos,currentPos()), std::optional<CompilationError>());
                        }
                        else if(token_string.compare("print")==0){
